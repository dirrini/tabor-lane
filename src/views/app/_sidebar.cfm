...
<aside class="workspace-sidebar">
    ...
    <nav hx-boost="true" hx-target="##workspace-main" hx-select="##workspace-main" hx-swap="outerHTML show:top" hx-push-url="true">
        ...
        <a href="/app/notifications" class="notification-nav-link #prc.page == 'notifications' ? 'active' : ''#" #prc.page == 'notifications' ? 'aria-current="page"' : ''#>
            <span><svg class="icon" aria-hidden="true"><use href="/resources/icons.svg##bell"></use></svg></span>
            #$r( "notifications.nav" )#
            <span class="notification-badge-poller"
                hx-get="/app/notifications/badge"
                hx-trigger="load, every 30s, notification-read from:body"
                hx-target="##notification-badge"
                hx-select="##notification-badge"
                hx-swap="outerHTML"
                hx-sync="this:drop"
                hx-history="false">
        ...
        <a href="/app/settings" class="#listFindNoCase( 'settings,integrations', prc.page ) ? 'active' : ''#" #listFindNoCase( 'settings,integrations', prc.page ) ? 'aria-current="page"' : ''#><span><svg class="icon" aria-hidden="true"><use href="/resources/icons.svg##settings"></use></svg></span>#$r( "app.settings" )#</a>
    </nav>
